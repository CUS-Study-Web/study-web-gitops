...
be-staging: 
	...
	podman-compose -f docker-compose.backend.staging.yml --env-file backend.env pull postgres redis

	@echo ">> Starting infrastructure with backend image: staging"
	podman-compose -f docker-compose.backend.staging.yml --env-file backend.env up -d --force-recreate

be-prod: 
	@echo ">> Pulling backend image"
	podman pull ghcr.io/cus-study-web/backend:latest
	@echo ">> Pulling auxiliary images Postgres, Redis..."
	podman-compose -f docker-compose.backend.prod.yml --env-file backend.env pull postgres redis

	@echo ">> Starting infrastructure with backend production"
	podman-compose -f docker-compose.backend.prod.yml --env-file backend.env up -d --force-recreate
